...
import ar.com.padelnec.gym.ui.GymNavigation;
...
public class MainLayout extends AppLayout implements AfterNavigationObserver {
    ...
    /** El club tiene el modulo de gimnasio prendido: agrega su grupo al menu. */
    private final boolean gymEnabled;
    ...
    public MainLayout(AuthenticationContext authenticationContext, AlertService alertService,
                      TenantService tenantService, AppProperties properties, GymModule gymModule,
                      NewBookingFeed newBookingFeed) {
        this.authenticationContext = authenticationContext;
        this.alertService = alertService;
        this.newBookingFeed = newBookingFeed;
        this.gymEnabled = gymModule.isEnabled();
    ...
    private VerticalLayout navigation() {
        ...
        // Modulo de gimnasio: solo aparece en los clubes que lo tienen prendido.
        if (gymEnabled) {
            drawer.add(GymNavigation.menu(
                    currentUser().map(ClubUserPrincipal::canManageSettings).orElse(false), this::iconChip));
        }
